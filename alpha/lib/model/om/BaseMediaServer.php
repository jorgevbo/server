...
/**
 * Base class that represents a row from the 'media_server' table.
 *
 * 
 *
 * @package Core
 * @subpackage model.om
 */
abstract class BaseMediaServer extends BaseObject  implements Persistent {
	...
	{
		if(!isset($this->oldColumnsValues[MediaServerPeer::ID]))
			$this->oldColumnsValues[MediaServerPeer::ID] = $this->id;
		...
		if ($this->id !== $v) {
			$this->id = $v;
			$this->modifiedColumns[] = MediaServerPeer::ID;
		}
	...
	{
		if(!isset($this->oldColumnsValues[MediaServerPeer::PARTNER_ID]))
			$this->oldColumnsValues[MediaServerPeer::PARTNER_ID] = $this->partner_id;
		...
		if ($this->partner_id !== $v) {
			$this->partner_id = $v;
			$this->modifiedColumns[] = MediaServerPeer::PARTNER_ID;
		}
	...
	public function setCreatedAt($v)
	{
		...
		if ( $this->created_at !== null || $dt !== null ) {
					...
					)
			{
				...
				$this->modifiedColumns[] = MediaServerPeer::CREATED_AT;
			}
	...
	public function setUpdatedAt($v)
	{
		...
		if ( $this->updated_at !== null || $dt !== null ) {
					...
					)
			{
				...
				$this->modifiedColumns[] = MediaServerPeer::UPDATED_AT;
			}
		} // if either are not null
	...
	{
		if(!isset($this->oldColumnsValues[MediaServerPeer::HOSTNAME]))
			$this->oldColumnsValues[MediaServerPeer::HOSTNAME] = $this->hostname;
		...
		if ($this->hostname !== $v) {
			$this->hostname = $v;
			$this->modifiedColumns[] = MediaServerPeer::HOSTNAME;
		}
	...
	{
		if(!isset($this->oldColumnsValues[MediaServerPeer::DC]))
			$this->oldColumnsValues[MediaServerPeer::DC] = $this->dc;
		...
		if ($this->dc !== $v) {
			$this->dc = $v;
			$this->modifiedColumns[] = MediaServerPeer::DC;
		}
	...
	public function setCustomData($v)
	{
		...
		if ($this->custom_data !== $v) {
			$this->custom_data = $v;
			$this->modifiedColumns[] = MediaServerPeer::CUSTOM_DATA;
		}
	...
	public function delete(PropelPDO $con = null)
	{
		...
		try {
			...
			if ($ret) {
				MediaServerPeer::doDelete($this, $con);
				$this->postDelete($con);
		...
		try {
			...
			for ($retries = 1; $retries < KalturaPDO::SAVE_MAX_RETRIES; $retries++)
			{
               $affectedRows = $this->doSave($con);
                if ($affectedRows || !$this->isColumnModified(MediaServerPeer::CUSTOM_DATA)) //ask if custom_data wasn't modified to avoid retry with atomic column 
                	break;
			...
			$this->postSave($con);
			MediaServerPeer::addInstanceToPool($this);
	...
	protected function doSave(PropelPDO $con)
	{
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->isModified()) {
				if ($this->isNew()) {
					$pk = MediaServerPeer::doInsert($this, $con);
					...
					$affectedObjects = MediaServerPeer::doUpdate($this, $con);
					if($affectedObjects)
						$this->objectSaved = true;
	...
	protected function doValidate($columns = null)
	{
		if (!$this->alreadyInValidation) {
			...
			if (($retval = MediaServerPeer::doValidate($this, $columns)) !== true) {
	...
	{
		$pos = MediaServerPeer::translateFieldName($name, $type, BasePeer::TYPE_NUM);
	...
	{
		$keys = MediaServerPeer::getFieldNames($keyType);
		$result = array(
			$keys[0] => $this->getId(),
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(MediaServerPeer::PARTNER_ID)) $criteria->add(MediaServerPeer::PARTNER_ID, $this->partner_id);
	...
	public function buildPkeyCriteria()
	{
		...
		$criteria->add(MediaServerPeer::ID, $this->id);
		
		if($this->alreadyInSave)
		{
			if ($this->isColumnModified(MediaServerPeer::CUSTOM_DATA))
			{
				if (!is_null($this->custom_data_md5))
					$criteria->add(MediaServerPeer::CUSTOM_DATA, "MD5(cast(" . MediaServerPeer::CUSTOM_DATA . " as char character set latin1)) = '$this->custom_data_md5'", Criteria::CUSTOM);
					//casting to latin char set to avoid mysql and php md5 difference
				else 
					$criteria->add(MediaServerPeer::CUSTOM_DATA, NULL, Criteria::ISNULL);
			}
			
			if (count($this->modifiedColumns) == 2 && $this->isColumnModified(MediaServerPeer::UPDATED_AT))
			{
				$theModifiedColumn = null;
				foreach($this->modifiedColumns as $modifiedColumn)
					if($modifiedColumn != MediaServerPeer::UPDATED_AT)
						$theModifiedColumn = $modifiedColumn;
						
				$atomicColumns = MediaServerPeer::getAtomicColumns();
	 ...
	 * objects.
	 *
	 * @param      object $copyObj An object of MediaServer (or compatible) type.
	 ...
	 *
	 * @param      boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
	 * @return     MediaServer Clone of current object.
	 * @throws     PropelException
	 */
	 ...
	 *
	 * @var     MediaServer Clone of current object.
	 */
	 ...
	 * Stores the source object that this object copied from 
	 *
	 * @param      MediaServer $copiedFrom Clone of current object.
	 */
	public function setCopiedFrom(MediaServer $copiedFrom)
	{
	 ...
	 *
	 * @return     MediaServerPeer
	 */
	public function getPeer()
	{
		if (self::$peer === null) {
			self::$peer = new MediaServerPeer();
		}
		return self::$peer;
	}
